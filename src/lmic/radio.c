...
static void rxlora (u1_t rxmode) {
    ...
    if (rxmode == RXMODE_RSSI) {
        ...
        u1_t bw = getBw(LMIC.rps);
        u1_t cr = getCr(LMIC.rps);
        lmic_printf("%u: %s, freq=%u, SF=%d, BW=%d, CR=4/%d, IH=%d\n",
               os_getTime(),
...
#ifdef CFG_sx1276_radio
    if (v != 0x12) {
        lmic_printf("Invalid radio version number: Expected 0x12, got 0x%x. Aborting.\n", v);
#elif CFG_sx1272_radio
    if (v != 0x22) {
        lmic_printf("Invalid radio version number: Expected 0x22, got 0x%x. Aborting.\n", v);
#else
#error Missing CFG_sx1272_radio/CFG_sx1276_radio
#endif
        hal_enableIRQs();
...
void radio_irq_handler (u1_t dio) {
    ...
    if( (readReg(RegOpMode) & OPMODE_LORA) != 0) { // LORA modem
        ...
        lmic_printf("%u: irq: dio: 0x%x flags: 0x%x\n", now, dio, flags);
#endif
